<x-app-layout>
    ...
    <div class="px-0 sm:px-2 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <!-- Backup Card -->
            <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100/50 hover:shadow-xl hover:shadow-blue-600/5 transition-all duration-500 group"
                 x-data="{ confirmBackup: false }">
                <div class="flex items-start justify-between mb-6">
                    <div class="h-14 w-14 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-500 group-hover:scale-110 group-hover:bg-blue-600 group-hover:text-white transition-all duration-500 shadow-sm">
                    ...
                    <span class="px-4 py-1.5 rounded-full bg-blue-50 text-blue-600 text-[10px] font-black uppercase tracking-widest border border-blue-100">Recommended</span>
                </div>
                <h4 class="text-xl font-black text-slate-800 mb-2">Backup Database</h4>
                <p class="text-sm text-slate-500 leading-relaxed mb-8">Download cadangan data sistem (.sql) untuk keamanan data transaksi Anda.</p>
                
                <button @click="confirmBackup = true" class="w-full py-4 bg-slate-50 text-slate-600 rounded-2xl font-bold text-sm hover:bg-blue-600 hover:text-white transition-all duration-300 border border-slate-100 flex items-center justify-center group/btn">
                ...
                <template x-teleport="body">
                    <div x-show="confirmBackup" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" x-cloak>
                        ...
                        <div class="relative bg-white rounded-[2.5rem] shadow-2xl w-full max-w-sm p-8 text-center" x-show="confirmBackup" x-transition:scale>
                            <div class="inline-flex items-center justify-center h-16 w-16 rounded-2xl bg-blue-50 text-blue-600 mb-6">
                            ...
                            <h3 class="text-2xl font-black text-slate-800 mb-2">Mulai Backup?</h3>
                            <p class="text-sm text-slate-500 mb-8">Proses ini akan mengunduh database terbaru ke perangkat Anda.</p>
            ...
            <div class="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100/50 hover:shadow-xl hover:shadow-rose-600/5 transition-all duration-500 group"
                ...
                <p class="text-sm text-slate-500 leading-relaxed mb-8">Hapus seluruh riwayat permintaan barang secara permanen.</p>
                ...
                <template x-teleport="body">
                    <div x-show="open" class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" x-cloak>
                        ...
                        <div class="relative bg-white rounded-[2.5rem] shadow-2xl w-full max-w-sm overflow-hidden border border-slate-100"
                             x-show="open" 
                             x-transition:enter="ease-out duration-300"
                             x-transition:enter-start="opacity-0 scale-95"
                             x-transition:enter-end="opacity-100 scale-100">
                            
                            <form action="{{ route('system.reset-requests') }}" method="POST" @submit="loading = true">
                                @csrf
                                <div class="p-8">
                                    <div class="text-center mb-6">
                                        <div class="inline-flex items-center justify-center h-20 w-20 rounded-3xl bg-rose-50 text-rose-600 mb-6 shadow-sm">
                                        ...
                                        <h3 class="text-2xl font-black text-slate-800 mb-2">Reset Seluruh Data?</h3>
                                        <p class="text-sm text-slate-500">Seluruh riwayat permintaan akan dihapus. Ketik <span class="text-rose-600 font-bold underline">RESET</span> untuk melanjutkan.</p>
                                    </div>
                                    ...
                                    <div class="mb-8">
                                        <input type="text" 
                                               ...
                                               placeholder="Tulis RESET di sini..." 
                                               class="w-full rounded-2xl border-2 border-slate-100 focus:border-rose-500 focus:ring-0 text-lg bg-slate-50 transition-all font-mono font-black uppercase text-center p-4">
                                    </div>

                                    <div class="flex justify-end gap-3 pt-4 border-t border-slate-50">
                                        <button @click="open = false; confirmation = ''" 
                                                type="button"
                                                class="px-6 py-3 bg-white border-2 border-slate-100 rounded-xl text-sm font-bold text-slate-500 hover:bg-slate-50 transition-all">
                                            Batal
                                        </button>
                                        <button type="submit" 
                                                :disabled="confirmation !== 'RESET' || loading"
                                                class="px-8 py-3 bg-rose-600 text-white rounded-xl text-sm font-bold shadow-lg shadow-rose-600/30 hover:bg-rose-700 transition-all disabled:opacity-20 flex items-center min-w-[120px] justify-center">
