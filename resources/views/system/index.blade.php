                ...
                <div class="p-8">
                    ...
                    <template x-teleport="body">
                        <div x-show="open" 
                            ...
                            <!-- Modal Content Box -->
                            <div class="relative bg-white rounded-2xl shadow-2xl transform transition-all w-full max-w-md overflow-hidden"
                                 x-show="open"
                                 x-transition:enter="ease-out duration-300"
                                ...
                                <div class="px-8 pt-10 pb-8 text-center sm:text-left">
                                    <div class="flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-5">
                                        <div class="flex-shrink-0 flex items-center justify-center h-16 w-16 rounded-full bg-rose-100 text-rose-600 mx-auto sm:mx-0">
                                        ...
                                        <div class="flex-1">
                                            <h3 class="text-2xl font-black text-slate-800 tracking-tight leading-none mb-4">Reset Seluruh Data?</h3>
                                            <p class="text-sm text-slate-500 leading-relaxed mb-6">
                                                Tindakan ini akan <span class="text-rose-600 font-bold underline">menghapus permanen</span> semua riwayat permintaan barang. Tindakan ini tidak dapat dibatalkan.
                                            </p>
                                            
                                            <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Konfirmasi Penghapusan</label>
                                            <input type="text" 
                                                   x-model="confirmation" 
                                                   @keydown.enter="if(confirmation === 'RESET') $refs.submitBtn.click()"
                                                   placeholder="Ketik 'RESET' untuk setuju" 
                                                   class="block w-full rounded-xl border-slate-200 focus:border-rose-500 focus:ring-rose-500 sm:text-sm bg-slate-50 transition-all font-mono font-bold uppercase text-center sm:text-left p-4">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="bg-slate-50 px-8 py-6 flex flex-col sm:flex-row-reverse gap-4">
                                    <form action="{{ route('system.reset-requests') }}" method="POST" class="w-full sm:w-auto">
                                        ...
                                        <button type="submit" 
                                                ...
                                                class="w-full inline-flex justify-center rounded-xl px-10 py-4 bg-rose-600 text-md font-bold text-white hover:bg-rose-700 shadow-lg shadow-rose-600/30 transition-all disabled:opacity-20 disabled:cursor-not-allowed uppercase tracking-wider">
                                            Lanjutkan
                                        </button>
                                    </form>
                                    <button @click="open = false; confirmation = ''" type="button" class="w-full sm:w-auto inline-flex justify-center rounded-xl px-10 py-4 bg-white border border-slate-200 text-md font-bold text-slate-600 hover:bg-slate-100 transition-all uppercase tracking-wider">
                                        Batal
                                    </button>
                                </div>
                            </div>
