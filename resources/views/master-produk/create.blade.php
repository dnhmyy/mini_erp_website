    ...
    <div class="max-w-2xl bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mx-auto">
        <div class="p-8">
            <form action="{{ route('master-produk.store') }}" method="POST" class="space-y-6" x-data="{ 
                ...
                <div class="space-y-6">
                    <div>
                        <label for="kategori" class="block text-sm font-bold text-slate-700 mb-1">Kategori Produk</label>
                        <select name="kategori" id="kategori" required class="block w-full border border-slate-200 rounded-xl py-2.5 px-4 focus:ring-brand-primary focus:border-brand-primary transition">
                            ...
                            @foreach($roles as $val => $label)
                                <label class="inline-flex items-center group cursor-pointer bg-white p-3 rounded-xl border border-slate-100 hover:border-brand-primary transition-all">
                                    <input type="checkbox" name="target_role[]" value="{{ $val }}" 
                                           ...
                                           {{ is_array(old('target_role')) && in_array($val, old('target_role')) ? 'checked' : '' }}>
