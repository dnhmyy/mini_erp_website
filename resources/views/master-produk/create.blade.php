<x-app-layout>
    ...
    <div class="max-w-2xl bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden mx-auto">
        <div class="p-8">
            <form action="{{ route('master-produk.store') }}" method="POST" class="space-y-6">
                @csrf
                <div>
                    <label for="kode_produk" class="block text-sm font-medium text-slate-700 mb-1">Kode Produk</label>
                    <input type="text" name="kode_produk" id="kode_produk" value="{{ old('kode_produk') }}" required class="block w-full border border-slate-200 rounded-lg py-2 px-3 focus:outline-none focus:ring-1 focus:ring-brand-primary focus:border-brand-primary sm:text-sm transition">
                ...
                <div>
                    ...
                    <input type="text" name="nama_produk" id="nama_produk" value="{{ old('nama_produk') }}" required class="block w-full border border-slate-200 rounded-lg py-2 px-3 focus:outline-none focus:ring-1 focus:ring-brand-primary focus:border-brand-primary sm:text-sm transition">
                ...
                <div id="target_role_wrap">
                    ...
                    <select name="target_role" id="target_role" class="block w-full border border-slate-200 rounded-lg py-2 px-3 focus:outline-none focus:ring-1 focus:ring-brand-primary focus:border-brand-primary sm:text-sm transition">
                        <option value="">-- Semua (kalau GA tidak perlu diisi) --</option>
                        ...
                        <option value="all" {{ old('target_role') == 'all' ? 'selected' : '' }}>Semua (Staff Admin & Staff Produksi)</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="satuan" id="satuan" value="{{ old('satuan', 'Pcs') }}" required class="block w-full border border-slate-200 rounded-lg py-2 px-3 focus:outline-none focus:ring-1 focus:ring-brand-primary focus:border-brand-primary sm:text-sm transition" placeholder="Contoh: Pcs, Box, Kg">
