    ...
    <div class="max-w-2xl bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mx-auto">
        <div class="p-8">
            <form action="{{ route('master-produk.update', $masterProduk) }}" method="POST" class="space-y-6" x-data="{ 
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div id="target_role_wrap">
                        ...
                        <select name="target_role" id="target_role" class="block w-full border border-slate-200 rounded-xl py-2.5 px-4 focus:ring-brand-primary focus:border-brand-primary transition">
                            ...
                            <option value="mixing" {{ old('target_role', $masterProduk->target_role) == 'mixing' ? 'selected' : '' }}>Mixing</option>
                            <option value="all" {{ old('target_role', $masterProduk->target_role) == 'all' ? 'selected' : '' }}>Semua (Semua Staff)</option>
                        </select>
                ...
                <div>
                    <label class="block text-sm font-bold text-slate-700 mb-3 text-center uppercase tracking-widest text-[11px] text-slate-400">Ketersediaan Cabang</label>
